...
步骤2: 日期事件特征分类（基于dim.dim_holidays表）
通过LEFT JOIN dim.dim_holidays表，实现精确的日期事件分类：

A. 工作日类型判断：
   - 正常工作日：周一至周五且不在节假日表中
   - 调休工作日：本来是周末但因节假日调休安排需要上班
     * 条件：h.is_workday = true（节假日表中标记为工作日）
     * 示例：2025-01-26(周日)、2025-02-15(周六)等调休日期
   - 识别逻辑：
     ```sql
     CASE 
         WHEN h.is_workday = true THEN 'workday_adjusted'  -- 调休工作日
         WHEN h.holiday_date IS NULL AND EXTRACT(DOW FROM timestamp) NOT IN (0,6) 
              THEN 'workday_normal'  -- 正常工作日
         ELSE 'non_workday'
     END
     ```
...
B. 周末类型判断：
   ...
   - 识别逻辑：
     ```sql
     CASE 
         WHEN h.holiday_date IS NULL AND EXTRACT(DOW FROM timestamp) IN (0,6) 
              THEN 'weekend_normal'  -- 正常周末
         WHEN h.is_workday = true AND EXTRACT(DOW FROM timestamp) IN (0,6)
              THEN 'weekend_cancelled'  -- 调休取消的周末
         ELSE 'non_weekend'
     END
     ```
...
C. 节假日类型判断：
   - 免费通行节假日：高速公路免费通行期间
     * 条件：h.is_free = true 且 h.is_workday = false
     * 包括：春节、劳动节、国庆节
     * 特征：交通流量显著激增，流量模式与平时差异极大
   
   - 收费节假日：高速公路正常收费的节假日
     * 条件：h.is_free = false 且 h.is_workday = false  
     * 包括：元旦、清明节、端午节、中秋节
     * 特征：流量有所增加但相对温和，主要体现返乡探亲需求

D. Pattern_type生成逻辑：
   根据上述分类生成用于基准计算的模式类型标识：
   ```sql
   CASE 
...
E. 详细特征标记：
   ...
   SELECT 
       ...
       -- 生成复合特征
       CASE 
           WHEN h.is_free = true THEN 'holiday_free'
           WHEN h.is_free = false AND h.holiday_date IS NOT NULL THEN 'holiday_nofree'
           WHEN h.is_workday = true THEN 'workday'
           WHEN h.holiday_date IS NULL AND EXTRACT(DOW FROM timestamp) NOT IN (0,6) THEN 'workday'
           WHEN h.holiday_date IS NULL AND EXTRACT(DOW FROM timestamp) IN (0,6) THEN 'weekend'
           ELSE 'workday'
       END as pattern_type,
       -- 附加特征标记
       CASE 
           WHEN h.is_workday = true AND EXTRACT(DOW FROM timestamp) IN (0,6) THEN true
           ELSE false
       END as is_adjusted_workday,  -- 是否为调休工作日
       CASE 
           WHEN h.is_free = true THEN true
           ELSE false  
       END as is_free_highway  -- 是否为免费通行期
   FROM data_table d
   LEFT JOIN dim.dim_holidays h ON DATE(d.timestamp) = h.holiday_date
   ```

步骤3: 时段标准化和完整性检查
- 将5分钟流量数据聚合为小时数据
- 按pattern_type进行分类聚合，确保不同日期类型分别处理
- 计算小时平均流量，考虑节假日流量激增的特殊性
- 标记数据完整性和时间覆盖度
- 验证调休日期的正确识别和分类
```
...
主要OD对（日均>100辆）:
- 使用全部约60天历史数据
- 按pattern_type分别建立基准(workday/weekend/holiday_free/holiday_nofree)
- 采用时间衰减权重 + 站点类型权重
- 最小数据点要求：square-square≥30个，其他≥25个
- 调休工作日数据归入workday类别进行基准计算

次要OD对（日均50-100辆）:
- 使用全部约60天历史数据  
- 工作日/周末/节假日分别建立基准，数据不足时可合并相近类型
- 采用时间衰减权重 + 站点类型权重
...
pattern_type: workday, weekend, holiday_free, holiday_nofree

```json
{
  "stationId": "string",
  "baseFlowPattern": {
    "workday": [                    // 工作日基准模式 (0-23小时，包含调休工作日)
      {"hour": 0, "baseFlow": 120.5},
      {"hour": 1, "baseFlow": 95.2},
      // ... 24个时段
    ],
    "weekend": [                    // 周末基准模式 (0-23小时，正常周末)
      {"hour": 0, "baseFlow": 85.3},
      // ... 24个时段
    ],
    "holiday_free": [               // 免费通行节假日基准模式 (春节、劳动节、国庆节)
      {"hour": 0, "baseFlow": 180.2},
      {"hour": 1, "baseFlow": 165.8},
      // ... 24个时段
    ],
    "holiday_nofree": [             // 收费节假日基准模式 (元旦、清明、端午、中秋)
      {"hour": 0, "baseFlow": 95.7},
      {"hour": 1, "baseFlow": 88.3},
      // ... 24个时段
    ]
  },
...
- **站点类型权重影响**: square-square类型基准权重最高，gantry-gantry类型权重较低
...
- v1.2 (2025-06-19): 新增站点类型差异化处理
  ...
  - 优化OD对标识生成和相似性计算算法
  - 完善数据库表结构和输出格式
- v1.3 (2025-07-03): 删除置信度相关内容
  - 删除基准流量输出格式中的confidence字段
  - 删除OD基准流量数据库表中的confidence字段
  - 删除OD基准流量JSON输出中的confidence字段
  - 删除基准可信度阈值相关的判别条件
  - 删除OD可信度阈值相关的参数和判别条件
  - 删除CONFIDENCE_THRESHOLD参数
  - 移除了基准可信度评估步骤中的可信度计算公式
  - 修改了站点类型权重影响的描述，从"可信度最高"改为"权重最高"
